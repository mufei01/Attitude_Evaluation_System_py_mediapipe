...
# import main   # 将pyqtUI导入
# from main import userPage
...
def process(flag,userpage):
    ...
    pose_samples_folder = 'mediapipe_knn/fitness_poses_csvs_out'
    ...
    while video_cap.isOpened():
        ...
        # 将输出帧转换为numpy数组
        array = np.array(output_frame)

        # 将BGR格式的numpy数组转换为Qt图像对象
        height, width, channel = array.shape
        bytesPerLine = 3 * width
        qt_image = QImage(array.data, width, height, bytesPerLine, QImage.Format_RGB888)

        # 将Qt图像对象转换为像素图
        pixmap = QPixmap.fromImage(qt_image)
        pixmap = pixmap.scaled(userpage.label_video_2.size())
        # 将像素图设置为Label的图像
        userpage.label_video_2.setPixmap(pixmap)
