...
def trainset_process(flag):
    ...
    if flag == 1:
        if os.path.isfile('mediapipe_knn/fitness_poses_csvs_out/push_up.csv') and os.path.isfile('mediapipe_knn/fitness_poses_csvs_out/push_down.csv'):
            return
    # 如果fitness_poses_csvs_out文件夹下的squat_down.csv和squat_up.csv已经存在，则不用导入样本图片再训练了
    elif flag == 2:
        if os.path.isfile('mediapipe_knn/fitness_poses_csvs_out/squat_up.csv') and os.path.isfile('mediapipe_knn/fitness_poses_csvs_out/squat_down.csv'):
            return
    # 如果fitness_poses_csvs_out文件夹下的pull_down.csv和pull_up.csv已经存在，则不用导入样本图片再训练了
    elif flag == 3:
        if os.path.isfile('mediapipe_knn/fitness_poses_csvs_out/pull_up.csv') and os.path.isfile('mediapipe_knn/fitness_poses_csvs_out/pull_down.csv'):
            return
